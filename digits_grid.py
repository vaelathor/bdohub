...
for n in names:
    try:
        im = Image.open(f'/tmp/d_{n}.png')
    except Exception:
        im = None
    imgs.append((n, im))
# grade 2 colunas x 4 linhas, cada célula com rótulo pequeno e o número
cols, rows = 2, 4
...
for i, (n, im) in enumerate(imgs):
    ...
    if im:
        im2 = im.copy()
        im2.thumbnail((cell_w-10, cell_h-30))
        grid.paste(im2, (x0+5, y0+26))
grid.save('digits_grid.png')
